        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-4 sm:p-6 mt-1 mb-1"> <!-- Ajustando as margens superior (mt-4) e inferior (mb-2) aqui -->
                ...
                <form method="post" action="{{ route('device.store') }}" class="space-y-6">
                    ...
                    <div>
                        <label for="nome" class="mb-1 block font-semibold text-gray-700">{{ __('Nome') }}</label>
                        <input id="nome" name="nome" type="text" class="px-2 py-1 block w-full border-gray-300 rounded-md" autocomplete="nome" />
                    </div>

                    <div>
                        <label for="descricao" class="mb-1 block font-semibold text-gray-700">{{ __('Descrição') }}</label>
                        <input id="descricao" name="descricao" type="text" class="px-2 py-1 block w-full border-gray-300 rounded-md" autocomplete="descricao" />
                    </div>

                    <div>
                        <label for="mac" class="mb-1 block font-semibold text-gray-700">{{ __('MAC') }}</label>
                        <input id="mac" name="mac" type="text" class="px-2 py-1 block w-full border-gray-300 rounded-md" autocomplete="mac" />
                    </div>

                    <div class="flex justify-end">
                        <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded-md">{{ __('Salvar') }}</button>
                    </div>
                </form>
            </div>
        </div>
